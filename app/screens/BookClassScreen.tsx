...
export default function BookClassScreen() {
  ...
  const getWeekDays = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const hasClasses = classes.some((cls) => cls.date.split("T")[0] === dateStr);
      const dayNames = isArabic
        ? ["ح", "ن", "ث", "ر", "خ", "ج", "س"]
        : ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];
      days.push({ date, dateStr, day: date.getDate(), dayName: dayNames[i], hasClasses });
    }
    return days;
  };

  const weekDays = getWeekDays();
  const currentMonth = currentWeekStart.toLocaleDateString(isArabic ? "ar" : "en", {
    month: "long", year: "numeric",
  });
  const classesForSelectedDate = classes.filter(
    (cls) => cls.date.split("T")[0] === selectedDate,
  );
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.calendarContainer}>
        ...
        <View style={s.weekContainer}>
          {weekDays.map((item) => {
            ...
            const dots = [];
            if (hasBooked) dots.push(<View key="booked" style={[s.dot, { backgroundColor: "#3B82F6" }]} />);
            if (hasUnbooked) dots.push(<View key="unbooked" style={[s.dot, { backgroundColor: "#FF7002" }]} />);
      ...
      {classesForSelectedDate.length > 0 ? (
          ...
          renderItem={({ item }) => (
            ...
            >
              ...
              >
                ...
                />
                <View style={[s.statusLabel, { backgroundColor: item.isBooked ? "#3B82F6" : "#FF7002" }]}>
                  <Text style={[s.statusText, {
                    textAlign: isArabic ? "right" : "left",
                    writingDirection: isArabic ? "rtl" : "ltr",
                  }]}>
                    {item.isBooked ? i18n.t("already_booked") : i18n.t("available")}
                  </Text>
                </View>
                <View style={s.cardContent}>
                  <Text style={s.classType}>{isArabic ? item.nameAr : item.nameEn}</Text>
                  <View style={s.infoRow}>
                    <MaterialCommunityIcons name="calendar" size={16} color="#FF7002" />
                  ...
                  <View style={s.infoRow}>
                    <MaterialCommunityIcons name="account-group" size={16} color="#FF7002" />
                    <Text style={[s.infoText, {
                      textAlign: isArabic ? "right" : "left",
                      writingDirection: isArabic ? "rtl" : "ltr",
                    }]}>
        ...
        <View style={s.center}>
          <Text style={[{ color: theme.emptyText, marginTop: 20 }, {
...
const createStyles = (theme: ReturnType<typeof getTheme>) =>
  StyleSheet.create({
    ...
    classType: { fontSize: 18, fontWeight: "700", color: "#FFFFFF", marginBottom: 6 },
