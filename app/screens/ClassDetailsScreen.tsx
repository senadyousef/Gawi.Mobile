...
export default function ClassDetailsScreen({ route }: any) {
  ...
  const handleBookClass = async () => {
    try {
      const MemberId = await AsyncStorage.getItem("MemberId");
      if (!MemberId) { Alert.alert("Login Required", "Please log in to book a class."); return; }
      const classes = await fetchGymClassByUser();
      const classToBook = classes.find((c: any) => c.id === classId);
      if (!classToBook) { Alert.alert("Error", "Class not found. Cannot book."); return; }
      if (classToBook.isBooked) { Alert.alert("Already Booked", "You have already booked this class."); return; }

      const isClassFull =
        classToBook.isFull === true ||
        classToBook.availableSeats === 0 ||
        (classToBook.capacity && classToBook.bookedCount && classToBook.bookedCount >= classToBook.capacity);

      if (isClassFull) { Alert.alert("Class Full", "Sorry, this class is fully booked."); return; }

      const payload = { userId: parseInt(MemberId), gymClassId: classToBook.id };
      ...
      if (!response.ok) { Alert.alert("Booking Failed", resultText || "Unable to book this class."); return; }
      Alert.alert("✅ Booking Confirmed", `You successfully booked ${classToBook.nameEn}!`);
  ...
  const handleBookClassDelete = async (userClassId: string) => {
    try {
      ...
      if (!response.ok) { alert("Unable to cancel this booking."); return; }
      alert("✅ Booking successfully cancelled");
      setGymClass((prev: any) => prev ? { ...prev, isBooked: false, userClassId: null } : prev);
    } catch (error) {
  ...
  return (
    <View style={s.container}>
      {/* Header Image */}
      <ImageBackground source={{ uri: gymClass.photoUrl }} style={s.bannerImage}>
        <LinearGradient colors={["transparent", "rgba(0,0,0,1)"]} style={s.imageOverlay} />
        <View style={s.headerContent}>
          <Text style={s.classType}>{isArabic ? gymClass.nameAr : gymClass.nameEn}</Text>
          <View style={s.infoRow}>
            <MaterialCommunityIcons name="calendar" size={18} color="#FF7002" />
            <Text style={[s.infoText, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
              {new Date(gymClass.date).toLocaleDateString(isAr ? "ar" : "en")} • {gymClass.from}:00–{gymClass.to}:00
            </Text>
          </View>
        </View>
      </ImageBackground>
      ...
      <ScrollView style={s.detailsSection} showsVerticalScrollIndicator={false}>
        <Text style={[s.sectionTitle, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
          📋 {i18n.t("class_information")}
        </Text>

        <View style={s.detailBox}>
          <View style={[s.detailItem, { flexDirection: isAr ? "row-reverse" : "row" }]}>
            <MaterialCommunityIcons name="account-group" size={20} color="#FF7002" />
            <Text style={s.detailText}>{i18n.t("capacity")}: {gymClass.capacity} {i18n.t("attendees")}</Text>
          </View>
          <View style={[s.detailItem, { flexDirection: isAr ? "row-reverse" : "row" }]}>
            <MaterialCommunityIcons name="calendar-check" size={20} color="#2563EB" />
            <Text style={s.detailText}>{i18n.t("booked")}: {gymClass.bookedCount} / {gymClass.capacity}</Text>
          </View>
          <View style={[s.detailItem, { flexDirection: isAr ? "row-reverse" : "row" }]}>
            ...
            <Text style={s.detailText}>
              {gymClass.isBooked ? i18n.t("already_booked") : i18n.t("not_booked")}
            </Text>
          </View>
        </View>

        <Text style={[s.description, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
      ...
      {gymClass.isBooked && (
        <TouchableOpacity onPress={() => handleBookClassDelete(gymClass.userClassId)} activeOpacity={0.9}>
          <LinearGradient
