...
export default function PTDetailsScreen() {
        ...
        { text: i18n.t("no") },
        {
          ...
          onPress: async () => {
            try {
              ...
              if (!response.ok) throw new Error(`Failed to cancel class: ${response.status}`);
              trainer.ptWithUserAllClassDto = trainer.ptWithUserAllClassDto.filter(
  ...
  const myClasses = trainer.ptWithUserAllClassDto?.filter((c: any) => c.isMyClass);
  const otherClasses = trainer.ptWithUserAllClassDto?.filter((c: any) => !c.isMyClass);

  const parseDateTime = (dateStr: string, timeStr: string) => {
    if (!timeStr) return new Date(dateStr);
    let hour = 0, min = 0, period = "AM";
    if (timeStr.includes(" ")) {
  ...
  const renderClassCard = (cls: any, isMyClass: boolean, index: number) => {
    ...
    return (
      <TouchableOpacity
        key={index}
        style={[s.classCard, isMyClass ? s.myClassCard : s.otherClassCard, { flexDirection: isAr ? "row-reverse" : "row" }]}
        onPress={() => isMyClass && handleCancelClass(cls)}
      >
        <View style={{ flex: 1 }}>
          <Text style={[s.classDate, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
            {formattedDate}
          </Text>
          <Text style={[s.classTime, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
            {formattedTime}
          </Text>
          {isMyClass && (
            <Text style={[s.tapText, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
    ...
    <ScrollView style={s.container}>
      ...
      <View style={s.infoContainer}>
        ...
        <View style={[s.section, { direction: isAr ? "rtl" : "ltr" }]}>
          ...
          <View style={s.daysContainer}>
            {trainer.ptDaysDto?.map((day: any, index: number) => (
              <View key={index} style={[s.dayCard, { flexDirection: isAr ? "row-reverse" : "row" }]}>
                ...
                />
                <View>
                  <Text style={[s.dayText, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
                    {day.day}
                  </Text>
                  <Text style={[s.timeText, { textAlign: isAr ? "right" : "left", writingDirection: isAr ? "rtl" : "ltr" }]}>
        ...
        <View style={s.section}>
          ...
          {myClasses?.length
            ? myClasses.map((cls: any, index: number) => renderClassCard(cls, true, index))
        ...
        <View style={s.section}>
          ...
          {otherClasses?.length
            ? otherClasses.map((cls: any, index: number) => renderClassCard(cls, false, index))
        ...
        >
          ...
          >
            <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16, textAlign: "center", writingDirection: isAr ? "rtl" : "ltr" }}>
...
const createStyles = (theme: ReturnType<typeof getTheme>) =>
  StyleSheet.create({
    ...
    sectionTitle: {
      ...
      marginBottom: 12, color: theme.ink, // 👈
    },
    ...
    classDate: {
      ...
      marginBottom: 4, color: theme.ink, // 👈
    },
    classTime: {
      fontSize: 13, fontWeight: "500",
      marginBottom: 2, color: theme.muted, // 👈
    },
    tapText: {
      fontSize: 12,
      color: theme.subtle,              // 👈
